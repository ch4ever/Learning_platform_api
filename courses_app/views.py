...
class CourseViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='create',
            permission_classes=[IsAuthenticated, TeacherOrAbove, VerifiedTeacher])
    ...
    @extend_schema(summary='request to join course',
                   ...
                   responses={201: OpenApiResponse(description='Request created'),
                              200: OpenApiResponse(description='Joined successfully'),
    ...
    def request_to_join_course(self, request, pk):
        ...
        serializer = CourseRequestSerializer(data=request.data,
                                             context={'request': request, 'course': course, 'user': user})
        serializer.is_valid(raise_exception=True)
    ...
    @action(detail=True, methods=['patch', 'get'], url_path='settings',
    ...
    @action(detail=True, methods=['post', 'get'], url_path='users',
    ...
    def course_users(self, request, pk):
        ...
        if request.method == "POST":
            ...
            serializer = CourseUserPromoteSerializer(context={'course': course, 'user': request_user},
                                                     data=request.data)
            serializer.is_valid(raise_exception=True)
    ...
    @extend_schema(summary='kick user from course',
                   ...
                   responses={200: OpenApiResponse(description='User kicked'),
                              404: OpenApiResponse(description='Course not found')},
                   parameters=[
                       OpenApiParameter(name='pk', location=OpenApiParameter.PATH, description='Course ID'), ], )
    ...
    @extend_schema(summary='manage requests',
                   request='',
                   responses={200: RequestsToCourseSerializer,
                              400: OpenApiResponse(description='Invalid request or status'),
                              404: OpenApiResponse(description='Course not found')},
    ...
    def manage_requests(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            return Response({'message': f"Task to {new_status} request has been accepted"},
...
class CourseBlocksViewSet(viewsets.ViewSet):
    ...
    @extend_schema(summary='Update block',
                   request=SectionContentCreateUpdateSerializer,
                   responses={200: OpenApiResponse(response={'oneOf': [SectionContentSerializer, RawTestSerializer]})},
    ...
    def partial_update(self, request, course_pk, section_pk, pk):
        ...
        if block.content_type == 'lection':
            serializer = SectionContentCreateUpdateSerializer(block, partial=True, data=request.data,
